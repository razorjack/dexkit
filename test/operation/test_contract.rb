...
class TestOperationContract < Minitest::Test
  ...
  def test_params_only
    op = operation(params: { age: Integer })
  ...
  def test_success_only
    op = operation(success: Integer)

    c = op.contract
    assert_equal({}, c.params)
    assert_equal Integer, c.success
    assert_equal [], c.errors
  end
  ...
  def test_params_types_support_case_equality
    op = operation(params: { count: Integer })

    type = op.contract.params[:count]
    assert type === 42, "expected type to match via ===" # rubocop:disable Style/CaseEquality
  end
  ...
  def test_pattern_matching
    ...
    assert_equal Integer, success
    assert_equal [:missing], errors
  end

  def test_to_h
    op = operation(success: String, errors: [:fail])
