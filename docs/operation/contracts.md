...
Use `description` to document what the operation does, and `desc:` on individual props for per-field documentation. These flow into contract introspection, JSON Schema export, and LLM tool definitions – see [Registry & Export](/tooling/registry) for the full story.
