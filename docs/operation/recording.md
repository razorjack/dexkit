...
Mongoid recording models work too. The `_id` field must be declared as `String` since dexkit passes string IDs generated by the pipeline – the `default` is a fallback for records created outside dexkit (e.g., in a console or seed script). Define matching fields and add the same sparse unique `once_key` index if you use `once`:
